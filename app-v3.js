...
const btnRefreshLocal = document.getElementById('btn-refresh-local');
...
// Força a raspagem direta do UOL via navegador
async function refreshNewsDirectly() {
    const icon = btnRefreshLocal.querySelector('i');
    ...
    try {
        ...
        if (!match) {
            // Tenta busca alternativa mais flexível caso mude o terminador de linha
            const altMatch = html.match(/__INITIAL_STATE__\s*=\s*(\{[\s\S]+?\})\s*</);
            if (!altMatch) throw new Error("Não foi possível localizar os dados do UOL na página.");
            match = altMatch;
        }
        ...
        // Scanner recursivo equivalente ao script python
        function scanRecursive(obj, isMain = false, isCarousel = false, isVideo = false, parentKey = "") {
            if (!obj) return;
            if (typeof obj === 'object') {
                if (Array.isArray(obj)) {
                    obj.forEach(val => scanRecursive(val, isMain, isCarousel, isVideo, parentKey));
                } else {
                    const component = obj.component || '';
                    const category = obj.category || '';
                    let currentIsMain = isMain;
                    if (component === 'CardHeadlineMain' || category === 'card-headline-main') {
                        currentIsMain = true;
                    }

                    let currentIsVideo = isVideo;
                    if (parentKey === 'headlineVideo' || obj.type === 'live' || obj.labelLive === 'AO VIVO') {
                        currentIsVideo = true;
                    }

                    const title = obj.title || obj.headline;
                    let link = obj.link || obj.url || obj.href || obj.linkMais;

                    if (title && link && typeof title === 'string' && typeof link === 'string') {
                        const linkLower = link.toLowerCase();
                        const isArticle = (
                            linkLower.includes('.htm') ||
                            linkLower.includes('.ghtm') ||
                            linkLower.includes('.shtml') ||
                            linkLower.includes('/noticias/') ||
                            linkLower.includes('/colunas/') ||
                            linkLower.includes('/ao-vivo/') ||
                            linkLower.includes('redirect-flash') ||
                            linkLower.includes('/reportagem/') ||
                            linkLower.includes('videos.uol.com.br')
                        );

                        const isSocialPlaylist = ['/social/', '/playlist/', '/videos/index', '/email/'].some(x => linkLower.includes(x));

                        if (isArticle && title.length > 20 && !isSocialPlaylist) {
                            let photoUrl = "";
                            ...
                            if (!photoUrl) {
                                for (const [k, v] of Object.entries(obj)) {
                                    if (['img', 'image', 'photo', 'thumb', 'pic'].some(x => k.toLowerCase().includes(x))) {
                                        if (typeof v === 'string' && (v.startsWith('http') || v.startsWith('//') || v.startsWith('/'))) {
                                            photoUrl = v;
                                            break;
                                        }
                                    }
                            ...
                            if (photoUrl && typeof photoUrl === 'string') {
                                if (photoUrl.startsWith('//')) {
                                    ...
                                    photoUrl = 'https://www.uol.com.br' + photoUrl;
                                }
                            } else {
                                photoUrl = "";
                            }
                            ...
                            let sourceLabel = 'UOL';
                            if (linkLower.includes('folha.uol.com.br')) {
                                sourceLabel = 'Folha';
                            }
                            if (currentIsVideo) {
                                sourceLabel = 'Canal UOL';
                            }

                            const relatedsList = [];
                            const relateds = obj.relateds;
                            if (Array.isArray(relateds)) {
                                relateds.forEach(rel => {
                                    const relTitle = rel.title || rel.headline;
                                    let relLink = rel.link || rel.url || rel.href;
                                    if (relTitle && relLink && typeof relTitle === 'string' && typeof relLink === 'string') {
                                        if (relLink.startsWith('//')) {
                                            relLink = 'https:' + relLink;
                                        } else if (relLink.startsWith('/')) {
                                            relLink = 'https://www.uol.com.br' + relLink;
                                        }
                                        relatedsList.push({
                                            title: relTitle.trim(),
                                            link: relLink.trim()
                    ...
                    for (const [k, val] of Object.entries(obj)) {
                        if (k === 'relateds') continue;
                        let childIsMain = currentIsMain;
                        let childIsCarousel = isCarousel;
                        let childIsVideo = currentIsVideo;

                        if (['mainHighlights', 'headlineHybrid'].includes(k)) {
                            childIsMain = true;
                        }
                        if (['headlineCollection', 'asideRows'].includes(k)) {
                            childIsCarousel = true;
                        }
                        if (k === 'headlineVideo') {
                            childIsVideo = true;
                        }

                        scanRecursive(val, childIsMain, childIsCarousel, childIsVideo, k);
                    }
                }
            }
        }
        ...
        // Consolida as duplicadas priorizando as que têm imagens e metadados
        const uniqueNewsDict = {};
        extractedItems.forEach(news => {
            const url = news.link;
            if (!uniqueNewsDict[url]) {
                uniqueNewsDict[url] = news;
            } else {
                if (news.photo && !uniqueNewsDict[url].photo) {
                    uniqueNewsDict[url].photo = news.photo;
                }
                if (news.is_main) uniqueNewsDict[url].is_main = true;
                if (news.is_carousel) uniqueNewsDict[url].is_carousel = true;
                if (news.is_video) uniqueNewsDict[url].is_video = true;
                if (news.relateds && news.relateds.length > 0 && (!uniqueNewsDict[url].relateds || uniqueNewsDict[url].relateds.length === 0)) {
                    uniqueNewsDict[url].relateds = news.relateds;
                }
            }
        });

        const newParsedNews = Object.values(uniqueNewsDict);
        let addedCount = 0;

        // Mescla no topo do newsData local as notícias que ainda não existiam
        newParsedNews.reverse().forEach(news => {
            const exists = newsData.some(existing => existing.link === news.link);
            if (!exists) {
        ...
        console.log(`Raspagem concluída! ${addedCount} novas notícias incorporadas ao feed.`);
        
        // Atualiza contadores e renderiza
        updateHistoryCount();
        updateSavedCount();
        renderFeed();

        alert(`${addedCount} novas notícias foram adicionadas ao topo do seu feed em tempo real!`);

    } catch (err) {
        console.error("Erro na raspagem direta:", err);
        alert(`Não foi possível buscar as notícias em tempo real: ${err.message}`);
    } finally {
        if (icon) icon.classList.remove('fa-spin');
        btnRefreshLocal.disabled = false;
    }
}

btnRefreshLocal.addEventListener('click', refreshNewsDirectly);
