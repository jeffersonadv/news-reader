...
// Evento do botão de atualização híbrido inteligente no cabeçalho
btnRefreshLocal.addEventListener('click', hybridNewsUpdate);

// Realiza o fluxo híbrido: tenta raspagem instantânea no navegador e usa o GitHub Actions como fallback
async function hybridNewsUpdate() {
    ...
    try {
        ...
        // Timeout de 3.5 segundos para a raspagem direta. Se demorar mais, cai no robô da nuvem
        ...
        alert(`${addedCount} novas notícias atualizadas instantaneamente no navegador!`);

    } catch (err) {
        console.log("Raspagem direta falhou/bloqueada. Acionando o robô da nuvem como fallback...", err);
        if (icon) icon.classList.remove('fa-spin');
        btnRefreshLocal.disabled = false;
        // Fallback para o Actions (exibe a tela azul de progresso)
