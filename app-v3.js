...
// Evento do botão de atualização híbrido no cabeçalho (tenta carregar rápido localmente; se falhar, abre o modal azul do GitHub Actions)
btnRefreshLocal.addEventListener('click', hybridNewsUpdate);

async function hybridNewsUpdate() {
    const icon = btnRefreshLocal.querySelector('i');
    if (icon) icon.classList.add('fa-spin');
    btnRefreshLocal.disabled = true;

    try {
        console.log("Iniciando raspagem direta ultra-rápida via Cloudflare/AllOrigins...");
        const proxyUrl = `https://api.allorigins.win/get?url=${encodeURIComponent('https://www.uol.com.br/')}&t=${Date.now()}`;
        
        // Timeout curto de 3.5 segundos para a raspagem direta no navegador
        ...
        if (!match) throw new Error("Dados não encontrados");
            ...
            if (typeof obj === 'object') {
                if (Array.isArray(obj)) {
                    ...
                    if (title && link && typeof title === 'string' && typeof link === 'string') {
                        ...
                        if (isArticle && title.length > 12 && !isIndex && !isBlacklistTitle) {
                            ...
                            if (photoUrl && typeof photoUrl === 'string') {
                                ...
                                else if (photoUrl.startsWith('/')) photoUrl = 'https://www.uol.com.br' + photoUrl;
                            } else {
                                photoUrl = "";
                            }
                            if (link.startsWith('//')) link = 'https:' + link;
                            else if (link.startsWith('/')) link = 'https://www.uol.com.br' + link;

                            let sourceLabel = 'UOL';
                            if (linkLower.includes('folha.uol.com.br')) sourceLabel = 'Folha';
                            if (currentIsVideo) sourceLabel = 'Canal UOL';
                            ...
                            if (Array.isArray(relateds)) {
                                relateds.forEach(rel => {
                                    ...
                                    if (relTitle && relLink && typeof relTitle === 'string' && typeof relLink === 'string') {
                                        if (relLink.startsWith('//')) relLink = 'https:' + relLink;
                                        else if (relLink.startsWith('/')) relLink = 'https://www.uol.com.br' + relLink;
                                        relatedsList.push({ title: relTitle.trim(), link: relLink.trim() });
                                    }
                                });
                            }
                    ...
                    for (const [k, val] of Object.entries(obj)) {
                        ...
                        scanRecursive(val, currentIsMain, isCarousel, currentIsVideo, k);
                    }
                }
            }
        }
        ...
        extractedItems.forEach(news => {
            const url = news.link;
            if (!uniqueNewsDict[url]) uniqueNewsDict[url] = news;
            else if (news.photo && !uniqueNewsDict[url].photo) uniqueNewsDict[url].photo = news.photo;
        });
        ...
        if (addedCount > 0) {
            // Renderiza localmente na hora
            updateHistoryCount();
            updateSavedCount();
            renderFeed();
            // Salva de forma silenciosa na nuvem para que outros navegadores também vejam os novos itens
            overwriteRepoSync().catch(err => console.error("Erro no sync silencioso:", err));
        } else {
            // Se não encontrou nenhuma notícia nova localmente, cai no robô remetendo à etapa 2 (nuvem)
            throw new Error("Nenhuma notícia nova na raspagem direta do navegador");
        }

    } catch (err) {
        console.log("Raspagem direta falhou/sem novidades. Abrindo modal do robô da nuvem como fallback...", err);
        if (icon) icon.classList.remove('fa-spin');
        btnRefreshLocal.disabled = false;
        // Abre e executa a etapa 2 com a tela azul de progresso
        await triggerGitHubUpdate();
    }
}
...
// Botão de Sincronização Manual — faz merge acumulativo (nunca apaga estado local)
btnForceSync.addEventListener('click', async () => {
    const icon = btnForceSync.querySelector('i');
    if (icon) icon.classList.add('fa-spin');
    btnForceSync.disabled = true;
    try {
        // Baixa os dados da nuvem e mescla com o estado local (união acumulativa)
        await loadSyncDataFromRepo();
        // Envia o estado consolidado de volta para a nuvem
        await syncWithRepo();
    } catch (err) {
